...
export function Sidebar({children}: {children: ReactNode}) {
  ...
  return <Box minH="100vh" bg="barber.900">
    <SidebarContent
    onClose={() => onClose}
  ...
  return (
    ...
    >
      <Flex h="20" alignItems="center" justifyContent="space-between" mx="8">
        <Link href="/dashboard">
            <Flex cursor="pointer" userSelect="none" flexDirection="row">
                ...
                <Text fontSize="2xl" fontFamily="monospace" fontWeight="bold" color="#F1B90A">Pro</Text>
            </Flex>
        </Link>
      ...
      {LinkItems.map(link => (
        <NavItem
          color="#fff"
        icon={link.icon} route={link.route} key={link.name}>
            {link.name}
        </NavItem>
      ))}
